...
def set_plot_environ(facecolor='#ffffff', grid_color=0.):
    ...
    plt.rcParams['axes.edgecolor'] = '#cccccc'  # '#DAA520'
...
def dist_chart(elements, x_label, y_label, title, f_name,
        colors=('#362cb2ff', '#c32148ff', '#fd5e0fff', '#228b22ff', '#C1A004ff', '#b710aaff'), bins=50, color_start=0, directories=None,
        scaleX=None):
    # ('#55a868ff', '#c44e52ff', '#85a8ffff', '#FFD700', '#FD5E0F', '#7897e6ff')
    ...
    # vals = ax.get_xticks()
    # ax.set_xticklabels(['{:0.2f}'.format(x) for x in vals])
...
def line_plot(data_sets, f_name=None, dir_name='../images/pngs/', labels=['a','b'], legend_loc=2,
        colors=('#FF00FF66', '#7DF9FF66'), sizes=(1., 5.), title=None, ylabel='y_lab', xlabel='x_lab', ylim=None,
        float_format=None):
    ...
    for d, mat in enumerate(data_sets):
        for y_ct, y in enumerate(mat):
            ...
            sns.lineplot(data=y, ci=None, color=colors[d], label=lbl, linewidth=sizes[d])
...
def cumul_return_plot(data_sets, f_name=None, dir_name='../images/pngs/', labels=['High volatility', 'Low volatility'], legend_loc=2,
        colors=('#FF00FF66', '#7DF9FF66'), sizes=(1., 5.), title=None):
    set_plot_environ(facecolor='#000000', grid_color=1.)
    ...
    for d, mat in enumerate(data_sets):
        ...
        for y_ct, y in enumerate(mat_cr_scaled):
            ...
            y_smooth = f(x_new)
            lbl = labels[d] if y_ct == 0 else None
            sns.lineplot(data=y_smooth, ci=None, color=colors[d], label=lbl, linewidth=sizes[d])

    pylab.xlabel('52-week time period')
    pylab.ylabel('Cumulative returns')
...
def scatter(elements, x_label='x label', y_label='y label', title='title', save_file=None, show=False, directories=None):
    set_plot_environ()
